...
class Complex:
    ...
    def __add__(self, another):
        return Complex(another.real + self.real, another.imaginary + self.imaginary)

    def __mul__(self, another):
        return Complex(self.real * another.real - self.imaginary * another.imaginary, self.real * another.imaginary + self.imaginary * another.real)
...
def make_to_string(front, mid, back, empty_repr):
    ...
    def mts(lk):
        ...
        a = lk.first
        if a != Link.empty:
            return front + str(a) + mid + mts(lk.rest) + back
    return mts
...
def prune_small(t, n):
    ...
    while len(t.branches) > n:
        a = t.branches[0].label
        j = 0
        for i in t.branches:
            if i.label > a:
                a = i.label
                j = t.branches.index(i)
        t.branches.pop(j)
    for i in t.branches:
        prune_small(i, n)
