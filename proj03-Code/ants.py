...
class Ant(Insect):
    ...
    def buff(self):
        ...
        if self.is_container and self.ant_contained:
            if not self.ant_contained.is_buffed:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while a.bees == []:
            ...
            if a.is_hive == True:
                return None
        return random_bee(a.bees)  # REPLACE THIS LINE
        # END Problem 3 and 4
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        if self.health <= 0:
            i = 0
            while i < len(a.bees):
                if self.damage >= a.bees[i].health:
                    a.bees[i].reduce_health(self.damage)
                else:
                    a.bees[i].reduce_health(self.damage)
                    i += 1
        # END Problem 5
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        """
        if self.scared > 0:
            if not (gamestate.time % 2 != 0 and self.slowed > 0):
                self.scared -= 1
