...
def swine_swap(player_score, opponent_score):
    ...
    """
    # BEGIN PROBLEM 4
    a=3**(player_score+opponent_score)
    b=a%10
    while a>=10:
        a//=10
    if a==b:
    ...
    def f(s0, s1,w=who):
        if w == 1:
            if s1-last_score>running_high:
                print(s1-last_score,"point(s)! That's a record gain for Player 1!")
                return announce_highest(w,s1,s1-last_score)
            return announce_highest(w,s1,running_high)
        else:
            if s0-last_score>running_high:
                print(s0-last_score,"point(s)! That's a record gain for Player 0!")
                return announce_highest(w,s0,s0-last_score)
            return announce_highest(w,s0,running_high)
    return f
    # END PROBLEM 7
...
def make_averaged(original_function, trials_count=1000):
    ...
    """
    # BEGIN PROBLEM 8
    def f(*args,of=original_function,tc=trials_count):
        i=0
        a=0
        while i<tc:
            i+=1
...
def max_scoring_num_rolls(current_score=0, dice=six_sided, trials_count=1000):
    ...
    s=0
    while i<11:
        s=make_averaged(roll_dice,trials_count)(current_score,i,dice)
        if b<s:
            b=s
            a=i
        i+=1
